module.exports = {
    ...
    async requestAddFriend(req, res) {
        ...
        try {
            ...
            let requestAdd = await FriendRequest.findOne({
                senderId: userId,
                receiverId
            })
    ...
    async confirmAddFriend(req, res) {
        ...
        try {
            ...
            let requestAdd = await FriendRequest.findOne({ senderId, receiverId: userId })
            if (!requestAdd) {
    ...
    async deleteFriend(req, res) {
        ...
        try {
            ...
            let requestAdd = await FriendRequest.findOne({ senderId: friendId, receiverId: userId })
            if (!requestAdd) {
                return res.json({ err: 401, message: `Invalid data request` })
            }
            if (requestAdd?.status == 'accept') {
